...
/**
 * Command class for the /fake-join command.
 *
 * @since 1.0.0
 */
@RequiredArgsConstructor
public final class FakeJoinCommand {

    private final CorePlugin plugin;

    // Registers the command
    public void register(final Commands registrar) {
        final var command = Commands.literal("fake-join")
                .requires(stack -> stack.getSender().hasPermission(CorePermission.COMMAND_VANISH.get()))
                .executes(this::execute)
                .build();

        registrar.register(command, "Create a fake join messages");
    }
    ...
    private int execute(final CommandContext<CommandSourceStack> context) {
        ...
        this.plugin.getPlayerController().vanishPlayer(corePlayer, player, true);

        // Send fake message
        final Component joinMessage = this.plugin.getConfigController().getMessage("server-join-message",
