...
/**
 * Command class for the /stafflist command.
 *
 * @since 1.0.0
 */
...
public final class StaffListCommand {
    ...
    public void register(final Commands registrar) {
        final var command = Commands.literal("stafflist")
                .requires(stack -> stack.getSender().hasPermission(CorePermission.COMMAND_STAFFLIST.get()))
                .executes(this::execute)
                .build();

        registrar.register(command, "List online staff");
    }
    ...
    private int execute(final CommandContext<CommandSourceStack> context) {
        ...
        if (!(components.isEmpty())) {
            sender.sendMessage(this.plugin.getConfigController().getMessage("command-stafflist",
                    Placeholder.component("list", Component.join(JoinConfiguration.commas(true), components))));
        } else {
            sender.sendMessage(this.plugin.getConfigController().getMessage("command-stafflist-empty"));
        }
